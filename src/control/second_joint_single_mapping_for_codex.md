...
- 驱动绳孔位半径：`R = 40 mm`
- 绕线轮半径：`r_w = 6.5 mm`
- 当前第二关节驱动绳限位位置：`284 mm`
- 后续装上第一关节后，第二关节限位位置变为：`287 mm`
- 现阶段控制验证只按 `284 mm` 计算
...
- `+y` 轴：沿相机 `+y` 正方向，也就是点一/1号舵机摆放方向
- `+x` 轴：俯视下相对点一顺时针 `90°`

> 这样定义的原因：当前相机 `+y` 正方向已经与点一/1号舵机方向一致，因此操作空间里的 `+y` 直接对应这个实物方向。
...
本文件只保留当前坐标定义：`+y` 轴对齐点一/1号舵机方向，代码中的角度 `alpha` 从 `+x` 轴开始、逆时针为正。机械装配编号角只作为换算输入，记为 `beta`，不作为控制代码的直接角度。
...
```math
Δl_4 = 40 \,\theta \cos\left(\phi - \frac{5\pi}{18}\right)
```

```math
Δl_5 = 40 \,\theta \cos\left(\phi + \frac{7\pi}{18}\right)
```

```math
Δl_6 = 40 \,\theta \cos\left(\phi - \frac{17\pi}{18}\right)
```
...
- `θ = 0`
- 第二关节竖直
- 摄像头动坐标初始与本文件定义的 `B={x,y,z}` 对齐，其中相机 `+y` 对应点一/1号舵机方向
...
- 图像目标偏左 / 偏右 → 调整 `x` 方向弯曲
- 图像目标偏上 / 偏下 → 调整 `y` 方向弯曲，当前 `+y` 已对齐点一/1号舵机方向
...
1. 当前只按 **第二关节单关节模型** 编程，长度取 `284 mm`
2. 坐标系 `+y` 轴 = **点一/1号舵机方向**，也就是当前相机 `+y` 正方向
3. 绳4、绳5、绳6的本地角位置分别为：
