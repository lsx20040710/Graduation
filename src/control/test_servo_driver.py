...
class HA8U25M_Servo:
    ...
    def set_zero_point(self, servo_id):
        """
        设置原点 (指令 0x17)
        将舵机当前位置设为原点，常用于装配后的零位校准，并为控制算法提供统一的运动起始参考。
        """
        # payload: id(1 Byte) + reset(1 Byte, 0x00)
        data = struct.pack('<BB', servo_id, 0x00)
        self._send_command(0x17, data)
        print(f"-> 已向 ID {servo_id} 下发设置原点指令，当前物理位置已重置为 0 度。")
    ...
    while True:
        ...
        while True:
            ...
            if choice == 'q':
                ...
                print(f"【警告】 即将把舵机 ID {target_id} 的当前物理位置强行设置为 0 度原点！")
                confirm = input("确定要设置原点吗？(y/n): ").strip().lower()
                if confirm == 'y':
                    servo.set_zero_point(target_id)
                else:
                    print("取消设置原点。")
